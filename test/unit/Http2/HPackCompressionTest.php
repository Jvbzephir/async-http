...
/**
 * @covers \KoolKode\Async\Http\Http2\HPackCompression
 */
class HPackCompressionTest extends TestCase
{
    ...
    {
        $encoded = (new HPackCompression())->compress($plain);
    ...
    {
        $decoded = (new HPackCompression())->decompress($this->compress($plain));
    ...
    {
        $this->assertEquals($plain, (new HPackCompression())->decompress($this->toBinary($compressed)));
    }
    
    public function testDetectsInvalidPadding()
    {
        $compressor = new HPackCompression();
    ...
    protected function compress(string $input): string
    {
        ...
        foreach (str_split($input, 1) as $char) {
            $len = HPackCompression::HUFFMAN_CODE_LENGTHS[ord($char)];
            $compressed .= sprintf("%0{$len}b ", HPackCompression::HUFFMAN_CODE[ord($char)]);
        }
