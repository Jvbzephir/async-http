...
class FrameTest extends \PHPUnit_Framework_TestCase
{
    ...
    {
        $frame = new Frame(Frame::DATA, 'Hello World', Frame::END_STREAM);
        ...
        $this->assertEquals('DATA [1] 11 bytes', (string) $frame);
    }
    ...
    {
        $frame = new Frame(Frame::DATA, chr(3) . 'foobar', Frame::PADDED);
    ...
    public function testCanEncodeFrame()
    {
        ...
        $frame = new Frame(Frame::DATA, $data, $flags);
        
        $this->assertEquals(substr(pack('NccN', strlen($data), Frame::DATA, $flags, 7), 1) . $data, $frame->encode(7));
    }
}
